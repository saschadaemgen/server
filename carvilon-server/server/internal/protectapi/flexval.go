...
// tolerant-decode helper internal/uaapi uses (Saison 21). The Protect
// Integration API is a foreign surface whose field types may drift
// across firmware revisions; typing an uncertain field as `string` or
// `*bool` would make ONE surprising value fail the whole /cameras or
// /sensors list decode and blank the Device Center categories. flexVal
// keeps the raw bytes and interprets them lazily on read, so the list
// always decodes and the field just renders as whatever the NVR sent.
package protectapi
