...
function injectStyles() {
  ...
  const css = `
  .mqc{display:flex;flex-direction:column;height:100%;min-height:0;font:12px/1.5 var(--mono,monospace)}
  ...
  host.innerHTML = `
    <div class="mqc">
      <div class="mqc-bar">
        ...
        <span class="mqc-login" data-login>
          <input data-url type="text" placeholder="ws://…" size="22" title="Broker-WebSocket-URL">
          <input data-user type="text" placeholder="Benutzer" size="10" autocomplete="off">
          <input data-pass type="password" placeholder="Passwort" size="10" autocomplete="off">
        </span>
        ...
        <a class="mqc-btn" href="/a/mqtt" target="_top" title="Broker-Einstellungen">⚙ Einstellungen</a>
      </div>
      <div class="mqc-bar">
        <input data-ptopic type="text" placeholder="Publish-Topic" size="20">
        <input data-pmsg type="text" placeholder="Nachricht" size="18">
        ...
        <span class="mqc-sp"></span>
        <input data-filter type="search" placeholder="filtern…" size="14">
      ...
      <div class="mqc-body"><div class="mqc-empty">Nicht verbunden. Login eintragen und „Verbinden". Tipp: Das Konsolen-Gerät braucht eine Subscribe-Regel auf <b>#</b> (in den Einstellungen), um allen Verkehr zu sehen.</div></div>
    </div>`;

  host.querySelector('[data-connect]').onclick = () => doConnect(host);
  host.querySelector('[data-publish]').onclick = () => doPublish(host);
  host.querySelector('[data-toggle-login]').onclick = () => host.querySelector('[data-login]').classList.toggle('hide');
