...
import { S, snap, vp, marquee, world, svg, NS, nodes, wires, selection, GRAPH } from './store.js';
...
vp.addEventListener('pointerdown',e=>{
  if(tool==='hand'){mode='pan';pan={sx:e.clientX,sy:e.clientY,tx:S.tx,ty:S.ty};vp.classList.add('panning');S.userAdjusted=true;try{vp.setPointerCapture(e.pointerId);}catch(_){}return;}
  ...
  if(nodeEl&&!e.target.closest('.port')){
    ...
    else{mode='pan';pan={sx:e.clientX,sy:e.clientY,tx:S.tx,ty:S.ty};vp.classList.add('panning');clearSel();}
  }
  if(mode){S.userAdjusted=true;try{vp.setPointerCapture(e.pointerId);}catch(_){}}
});
...
function updateWireTemp(e){if(!wiring)return;const wr=world.getBoundingClientRect();const a=socketCenter(wiring.sel,wr);if(!a)return;const cx=(e.clientX-S.tx)/S.scale,cy=(e.clientY-S.ty)/S.scale,dx=Math.max(40,Math.abs(cx-a.x)*0.5),from=wiring.dir==='out';
